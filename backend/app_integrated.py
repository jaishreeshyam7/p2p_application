...
class RobustAnalyzer:
    def __init__(self):
        ...
        if AUDIO_PIPELINE_AVAILABLE:
            try:
                ...
                print("✅ Audio transcriber, emotion, and confidence analyzers initialized")
            except Exception as e:
                print(f"⚠️ Failed to initialize audio pipeline: {e}")
        ...
        self.current_emotion = "Neutral"
        self.current_emotions = {
            'Happiness/Joy': 0, 'Trust': 50, 'Fear/FOMO': 0, 'Surprise': 0, 'Anger': 0
        }
        self.gaze_x = 50
        self.gaze_y = 50
        self.last_deepface_time = 0
    ...
    # === NEW COMPREHENSIVE AUDIO PROCESSING ===
    def process_audio_file(self, file_path):
        """Process a saved audio file for transcription, metrics, and confidence"""
        results = {}
        
        # 1. Transcribe (Gets Text, WPM, and Pauses)
        if self.audio_transcriber:
            try:
                transcription = self.audio_transcriber.transcribe(file_path)
                if isinstance(transcription, dict):
                    results['text'] = transcription.get('full_text', '')
                    
                    # Safely extract metrics AND PAUSES
                    metrics = transcription.get('metrics', {})
                    pauses = transcription.get('pauses', [])
                    
                    results['voice_metrics'] = {
        ...
        # 2. Text Emotion
        if self.emotion_analyzer and results.get('text'):
            try:
                results['text_emotions'] = self.emotion_analyzer.analyze(results['text'])
            except Exception as e:
                print(f"❌ Text emotion error: {e}")

        # 3. Audio Confidence (Pitch & Volume Stability)
        if self.confidence_analyzer:
            try:
                conf_metrics = self.confidence_analyzer.analyze(file_path)
                results['confidence'] = conf_metrics
            except Exception as e:
    ...
    def analyze_text_emotion(self, text):
        """Analyze emotion from text for manual input"""
        if self.emotion_analyzer:
            try:
    ...
    def bandpass_filter(self, signal, fs=30, low=0.7, high=3.5):
        try:
            ...
            low_norm = low / nyquist
            high_norm = high / nyquist
            if low_norm >= 1 or high_norm >= 1: return signal
            b, a = butter(3, [low_norm, high_norm], btype="band")
            return filtfilt(b, a, signal)
        except:
            return signal
    ...
    def calculate_heart_rate(self, signal):
        if len(signal) < self.fs * 6: return None
        try:
            sig_array = np.array(signal)
            sig_array = sig_array - np.mean(sig_array)
            std_val = np.std(sig_array)
            if std_val == 0: return None
            sig_array = sig_array / std_val
        ...
        try:
            ...
            for x_norm, y_norm, size in rois:
                ...
                if x2 > x1 and y2 > y1:
                    ...
                    if roi.size > 100: signals.append(np.mean(roi))
            return np.mean(signals) if signals else None
        except:
            return None
    ...
    def analyze_emotions_deepface(self, frame_rgb):
        try:
            ...
            emotions['Trust'] = max(10, min(90, emotions['Trust']))
            dominant = max(emotions.items(), key=lambda x: x[1])
            return emotions, dominant[0] if dominant[1] > 30 else "Neutral"
        except Exception as e:
            return None, None
    
    def detect_emotions_geometric(self, frame, face_landmarks):
        emotions = {'Happiness/Joy': 0, 'Trust': 50, 'Fear/FOMO': 0, 'Surprise': 0, 'Anger': 0}
        try:
            h, w = frame.shape[:2]
            left_mouth, right_mouth = face_landmarks.landmark[61], face_landmarks.landmark[291]
            ...
            left_eyebrow, right_eyebrow = face_landmarks.landmark[70], face_landmarks.landmark[300]
            ...
            if mouth_width > 25 and mouth_curve < -0.01:
                ...
                if mouth_curve > 0.005:
                    emotions['Anger'] = 60; emotions['Trust'] = 10; dominant = "Anger"
                else:
                    emotions['Fear/FOMO'] = 50; emotions['Trust'] = 20; dominant = "Fear/FOMO"
            else:
                emotions['Trust'] = 60; dominant = "Trust"
            return emotions, dominant
        except Exception as e:
            return emotions, "Neutral"
    ...
    def calculate_gaze(self, landmarks):
        try:
            left_eye, right_eye = landmarks.landmark[33], landmarks.landmark[263]
            return ((left_eye.x + right_eye.x) / 2) * 100, ((left_eye.y + right_eye.y) / 2) * 100
        except:
            return 50, 50
...
def handle_disconnect():
    print(f'❌ Client disconnected: {request.sid}')
    session_manager.waiting_users = [u for u in session_manager.waiting_users if u['socket_id'] != request.sid]
...
def handle_transcript(data):
    try:
        ...
        if analyzer.emotion_analyzer and text:
            text_emotions = analyzer.analyze_text_emotion(text)
            if text_emotions:
                emit('text-emotion', {'emotions': text_emotions, 'text': text}, room=room_id)
        
        objections = ['expensive', 'costly', 'think about it', 'not sure', 'too much', 'price']
        if any(word in text.lower() for word in objections):
            suggestion = f"💡 Objection detected: '{text}'. Consider addressing value proposition and ROI benefits."
            emit('ai-response', {'suggestion': suggestion, 'type': 'objection', 'timestamp': time.time()}, room=room_id)
            
        if room_id:
            emit('new-transcript', {'speaker': speaker, 'text': text, 'timestamp': time.time()}, room=room_id, include_self=False)
...
def handle_audio_chunk(data):
    print("\n" + "="*40)
    print("🎤 NEW AUDIO CHUNK RECEIVED")
    temp_path = None
    wav_path = None
    try:
        ...
        if not audio_b64:
            print("❌ ABORT: Frontend sent empty audio data")
            return

        audio_bytes = base64.b64decode(audio_b64)
        print(f"📦 Payload size: {len(audio_bytes)} bytes")
        
        if len(audio_bytes) < 1000: 
            print("❌ ABORT: Audio chunk is too small/empty.")
            return
        ...
        print(f"💾 Step 1: Saved WebM to {temp_path}")
        
        # 2. FFmpeg Conversion
        wav_path = temp_path + ".wav"
        try:
            print("⚙️ Step 2: Running FFmpeg conversion...")
            subprocess.run([
                'ffmpeg', '-y', '-i', temp_path, 
                '-ar', '16000', '-ac', '1', 
                '-loglevel', 'error', wav_path
            ], check=True)
            print(f"✅ FFmpeg success! Saved to {wav_path}")
        except FileNotFoundError:
            print("🚨 CRITICAL ERROR: FFMPEG NOT FOUND ON SYSTEM!")
            print("👉 Fix: Run 'sudo apt-get install ffmpeg' in your terminal")
            emit('error', {'message': 'FFmpeg is missing on the server!'}, room=room_id)
            return
        ...
        # 3. AI Analysis
        print("🧠 Step 3: Running AI Analysis (Whisper & Librosa)...")
        results = analyzer.process_audio_file(wav_path)
        print(f"📊 AI generated these keys: {list(results.keys())}")
        
       # ... (Steps 1, 2, 3 remain the same above this)
        results = analyzer.process_audio_file(wav_path)
        timestamp = time.time()
        
        # 4. Emit Transcription
        if results.get('text'):
            emit('transcription-result', {
                'text': results['text'],
                'speaker': 'User',
        ...
        # 5. Emit Confidence & Stability Data (FORCING REACT KEYS)
        if results.get('confidence'):
            conf_data = results['confidence']
            # We use .get() fallbacks to ensure React gets exactly what it asked for
            formatted_confidence = {
                'confidence_score': conf_data.get('confidence_score', conf_data.get('confidence', 0)),
                'pitch_stability': conf_data.get('pitch_stability', conf_data.get('pitch', 0)),
                'volume_stability': conf_data.get('volume_stability', conf_data.get('volume', 0))
            }
            emit('confidence-update', formatted_confidence, room=room_id)
        
        # 6. Emit WPM & Pauses Data
        if results.get('voice_metrics'):
            emit('voice-metrics', results['voice_metrics'], room=room_id)
            
        # 7. Emit Emotion Data (FIXING EVENT NAME & FINDING DOMINANT)
        if results.get('text_emotions'):
            emotions = results['text_emotions']
            dominant_emotion = "Neutral"
            
            # Figure out the highest scoring emotion to send to React
            if isinstance(emotions, dict) and emotions:
                dominant_emotion = max(emotions.items(), key=lambda x: x[1])[0]
            elif isinstance(emotions, list) and emotions and isinstance(emotions[0], dict):
                # Handles DistilBERT array formats
                dominant_emotion = max(emotions, key=lambda x: x.get('score', 0)).get('label', 'Neutral')
                
            # React is listening for 'emotion-update', not 'text-emotion'
            emit('emotion-update', {
                'emotions': emotions,
...
# ... (rest of the code remains the same)@socketio.on('analyze-context')
def handle_analysis(data):
    try:
        hr = data.get('hr', 0)
        room_id = data.get('roomId')
        ...
        if hr > 80: suggestions.append("⚡ High heart rate detected. Prospect may be stressed or excited.")
        
        trust_level = emotions.get('Trust', 0)
        if trust_level > 60: suggestions.append("✅ Strong trust signals. Good time to move toward commitment.")
        elif trust_level < 30: suggestions.append("⚠️ Low trust detected. Focus on building rapport.")
        
        if not suggestions: suggestions.append("📊 Conversation flow is normal. Continue with current approach.")
        
        emit('ai-response', {'suggestion': ' '.join(suggestions), 'type': 'analysis', 'timestamp': time.time()}, room=room_id)
    except Exception as e:
        print(f'❌ Error in analysis handler: {e}')

@socketio.on('session-started')
def handle_session_start(data):
    join_room(data.get('roomId'))
...
def handle_video_frame(data):
    ...
    try:
        frame_data = data.get('frame')
        room_id = data.get('roomId')
        if not LIBRARIES_AVAILABLE or not frame_data: return
        ...
        if results.multi_face_landmarks:
            ...
            if analyzer.use_deepface:
                emotions, dominant = analyzer.analyze_emotions_deepface(frame_rgb)
                if emotions is None: emotions, dominant = analyzer.detect_emotions_geometric(frame_rgb, landmarks)
            else:
                emotions, dominant = analyzer.detect_emotions_geometric(frame_rgb, landmarks)
            
            roi_signal = analyzer.extract_roi_signal(frame_rgb, landmarks)
            if roi_signal is not None: analyzer.signal_buffer.append(roi_signal)
            
            hr = None
            if len(analyzer.signal_buffer) >= analyzer.fs * 6:
                hr = analyzer.calculate_heart_rate(analyzer.signal_buffer)
                if hr: analyzer.current_hr = hr
...
@app.route('/health')
def health_check():
    return {'status': 'healthy', 'audio_pipeline': 'Active' if AUDIO_PIPELINE_AVAILABLE else 'Inactive'}
