...
def analyze_text_with_pipeline(text):
    """Analyze text using audio pipeline modules"""
    results = {
        'emotions': {},
        'objections': [],
        'sentiment': 'neutral'
    ...
    if AUDIO_PIPELINE_AVAILABLE:
        try:
            emotion_results = analyze_text_emotion(text)
            results['emotions'] = emotion_results
            
            objections = detect_objections(text)
            results['objections'] = objections
            
        except Exception as e:
            print(f"❌ Text analysis error: {e}")
    else:
        # Fallback
        objection_keywords = ['expensive', 'costly', 'think about it', 'not sure', 'too much', 'budget', 'price']
...
def handle_transcript(data):
    try:
        ...
        # Send objection alerts
        if analysis['objections']:
            objection_text = ', '.join(analysis['objections'])
            suggestion = f"💡 Objection detected: '{objection_text}'. Address with value proposition."
            emit('ai-response', {
...
def handle_analysis(data):
    try:
        ...
        if hr > 80:
            suggestions.append("⚡ High heart rate. Prospect may be stressed.")
        elif hr > 0 and hr < 60:
            suggestions.append("😌 Low heart rate. Prospect is calm.")
        
        trust_level = emotions.get('Trust', 0)
        if trust_level > 60:
            suggestions.append("✅ Strong trust signals.")
        elif trust_level < 30:
            suggestions.append("⚠️ Low trust. Build rapport.")
        
        anger_level = emotions.get('Anger', 0)
        if anger_level > 40:
            suggestions.append("🚨 Frustration detected.")
        
        fear_level = emotions.get('Fear/FOMO', 0)
        if fear_level > 40:
            suggestions.append("😰 Anxiety detected.")
        
        joy_level = emotions.get('Happiness/Joy', 0)
        if joy_level > 60:
            suggestions.append("😊 Positive engagement!")
        ...
        if not suggestions:
            suggestions.append("📊 Normal flow. Continue.")
...
def health_check():
    return {
        ...
        'mode': 'CV/ML' if LIBRARIES_AVAILABLE else 'Simulation',
        'emotion': 'DeepFace' if DEEPFACE_AVAILABLE else 'Geometric' if LIBRARIES_AVAILABLE else 'Simulated',
        'audio': 'Active' if AUDIO_PIPELINE_AVAILABLE else 'Disabled',
        'timestamp': time.time()
    }
...
if __name__ == '__main__':
    ...
    print("=" * 60)
    print("🌍 InsightEngine P2P Backend - FIXED VERSION")
    print("=" * 60)
    print(f"📡 Server: http://0.0.0.0:5000")
    print(f"🔬 Mode: {'CV/ML' if LIBRARIES_AVAILABLE else 'Simulation'}")
    print(f"💓 Heart Rate: {'✅' if LIBRARIES_AVAILABLE else '❌'}")
    print(f"😊 Emotions: {'✅ DeepFace' if DEEPFACE_AVAILABLE else '✅ Geometric' if LIBRARIES_AVAILABLE else '❌'}")
    print(f"👁️ Gaze: {'✅' if LIBRARIES_AVAILABLE else '❌'}")
    print(f"🎙️ Audio: {'✅' if AUDIO_PIPELINE_AVAILABLE else '❌'}")
    print("=" * 60)
